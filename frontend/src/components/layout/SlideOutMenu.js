                    ...
                    <nav className="space-y-2">
                        {navItems.map(item => (
                             <a key={item.id} href="#" onClick={(e) => { e.preventDefault(); navigateTo(item.id); }}
                               className={`block px-4 py-2 rounded-md transition-colors duration-200 ${activeView === item.id ? 'bg-sky-100 text-sky-900 font-bold' : 'text-gray-700 hover:bg-gray-100'}`}>{item.name}</a>
                        ))}
                    </nav>
                </div>
            </div>
